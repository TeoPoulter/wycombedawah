...
The production-ready V1 is implemented and connected to Supabase. The host, player and administrator experiences use the official WMC palette and artwork with responsive studio lighting, transitions, animated game states and dedicated small-screen layouts. Separate devices can join through a PIN-prefilled QR link, the large PIN is directly copyable, names are unique per room, submissions are final, each player gets one pass except at 1%, and eliminated players stay connected as spectators.
